...
class Search_by_index: #главный класс для индекса
    ...
    def add_index(self):
        ...
        with open(path, 'r') as file_stream:
            ...
            i = 0
            for item in data:
                self.es.index(index = 'games', id = i, body = item)
                i += 1

    def add_synonyms(self, query):
        tmp_list = list()
        result_tokens = self.es.indices.analyze(index='games', body={
            'analyzer': 'default',
            'text': [query]
        })
    ...
    def find_response(self, option, request):
        fields_list = [['title'], ['body'], ['title', 'body'], ['price']]
        ...
        return self.es.search(index = 'games', body = bodyItem)
...
if __name__ == '__main__':
    ...
    while (1):
        n = input('Введите n, чтобы:\n' +
              '1 - провести поиск по названию игры\n' +
              '2 - провести поиск по описанию\n' + 
              '3 - провести поиск по названию и описанию\n' +
              '4 - провести поиск по цене\n' +
              '5 - выйти\n')
        if n in ['1', '2', '3', '4']:
            ...
            if res_len > 0:
                ...
                for i in range(res_len):
                    ...
                    table.rows.append([number, name, score, price, URL])
                table.columns.header = ['Номер', 'Название игры', 'Рейтинг', 'Цена', 'URL']
                print(table)
        else:
            if (n != '5'):
